...
### Added
- 新增 `docs/13_寵物管理/晶片表.md`：晶片欄位規格、驗證規則、登記狀態機，以及示範租戶「小小貓屋」的晶片示範資料表。
- 於 `晶片表.md` 新增「轉讓登記紀錄（橘映之境工作坊）」區段，收錄四筆寵物登記證明（轉讓）資料（A–D），並同步更新 `晶片表.docx`。
- 於 `晶片表.docx` 新增「寵物買賣紀錄及晶片使用情形表（115 年）」章節（橫向版面，72 筆買賣/晶片使用紀錄）。
